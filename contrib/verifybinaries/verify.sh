...
###   adnetcoincore.org and adnetcoin.cash and compares them.
...
WORKINGDIR="/tmp/adnetcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://adnetcoincore.org"
HOST2="https://adnetcoin.cash"
BASEDIR="/bin/"
VERSIONPREFIX="adnetcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'adnetcoin-',
...
if [ $? -ne 0 ]; then
   echo "adnetcoin.cash failed to provide signature file, but adnetcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "adnetcoin.cash and adnetcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Adnetcoin Core binary release signing key installed?"
   fi
