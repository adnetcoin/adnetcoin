...
The RPM spec file provided here is for Adnetcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://adnetcoin.cash/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Adnetcoin git repository](https://github.com/adnetcoin/adnetcoin) but are not part of
...
Currently only `Source10` is used, for the example `adnetcoin.conf` file.
...
in the adnetcoin git repository. At present only an SVG version of the Adnetcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the adnetcoin
...
needed when 0.12.1 is released, a proper fix is already in the Adnetcoin git
master branch.
...
idea to build Adnetcoin Core against a static version of that library compiled
...
The `adnetcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `adnetcoin-server` package. At this point, most RPM based
...
The `adnetcoin-server` package should have SELinux support. How to properly do
...
Adnetcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/adnetcoin/). He has been
packaging Adnetcoin for Fedora at least since 2012.
...
4. I renamed the `adnetcoin` package that contains the Qt GUI to `adnetcoin-core` as
that appears to be how the general population refers to it, in contrast to
`adnetcoin-xt` or `adnetcoin-classic`. I wanted to make sure the general population
