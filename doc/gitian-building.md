...
*Setup instructions for a Gitian build of Adnetcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Adnetcoin
...
- [Building Adnetcoin Core](#building-adnetcoin-core)
...
To build Adnetcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
...
ADNETPATH=/some/root/path/adnetcoin
SIGPATH=/some/root/path/adnetcoin-detached-sigs

./bin/gbuild --url adnetcoin=${ADNETPATH},signature=${SIGPATH} ../adnetcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
